...
model = ensemble.RandomForestClassifier(
    random_state=42, min_samples_leaf=25
)
...
model_pipeline = pipeline.Pipeline(
    [("One Hot Encode", onehot), ("Modelo", grid)]
)
...
# Captura a data máxima da coluna de referência do dataset
data_maxima = pd.to_datetime(df["dtRef"]).max()
...
model_series = pd.Series(
    {
        ...
        "features": features,
        "metrics": df_metrics,
        "dt_train": data_maxima,  # Substituído para refletir a data máxima da base
    }
)

model_series.to_pickle("../../models/rf_fim_curso.pkl")
